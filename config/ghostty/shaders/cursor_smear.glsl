...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float sdfCurrentCursor = sdRectangle(normCoord, currentCenter, halfCurrentSize);
     ...
     float gray = dot(iCurrentCursorColor, vec4(0.299, 0.587, 0.114, 0.0));
     vec4 trail = mix(vec4(gray), iCurrentCursorColor, 2.5);

     // Blend trail color with background
     vec4 originalColor = fragColor;
     const float OPACITY = 0.6;
     fragColor.rgb = mix(fragColor.rgb, trail.rgb, alpha * OPACITY);
