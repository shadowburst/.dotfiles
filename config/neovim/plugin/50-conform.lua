...
Snacks.keymap.set("n", "<leader>cf", function() require("conform").format() end, { desc = "Format buffer" })
Snacks.keymap.set("n", "<leader>cl", function()
  if vim.tbl_contains({ "php" }, vim.bo.filetype) then
    return require("conform").format({ formatters = { "mago_lint" } })
  end
  if
    vim.tbl_contains({
      "javascript",
  ...
  then
    return require("conform").format({ formatters = { "oxlint" } })
  end
