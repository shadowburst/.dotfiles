...
class BrowserRuntime {
  ...
  async demonstrate(message: string, viewport: ViewportPreset, signal?: AbortSignal) {
    return demonstrate(await this.selectedPage(), { message, viewport, signal });
  }
  ...
  async record(
    ...
    return recordScript(await this.selectedPage(), script, options);
  }
...
export default function browserTools(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "browser_demonstrate",
    label: "Browser Demonstrate",
    description: "Record a manual workflow in the selected tab and return an editable page-scoped Puppeteer script for browser_record",
    parameters: Type.Object({
      message: Type.Optional(Type.String({ description: "Instruction shown beside Finish and Cancel" })),
      viewport: Type.Optional(StringEnum(VIEWPORTS, { description: "Desktop 1280x720 or iPhone 13 mobile emulation" })),
    }),
    async execute(_id, { message, viewport }, signal) {
      const result = await runtime.run(() => runtime.demonstrate(
        message ?? "Perform the workflow, then finish the recording.",
        viewport ?? "desktop",
        signal,
      ));
      return { content: [{ type: "text", text: await jsonOutput(result, "browser-demonstrate") }], details: result };
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Run one page-scoped Puppeteer script and record a paced visual-only WebM; use page for reads/assertions and act.goto/click/type/check/select for visible actions; use await new Promise(resolve => setTimeout(resolve, ms)) for extra waits because page.waitForTimeout is unavailable",
    parameters: Type.Object({
      script: Type.String({ description: "Async function body with page and cursor-aware act helpers in scope" }),
      name: Type.Optional(Type.String({ description: "Safe filename in Pi's managed recording cache; .webm is optional" })),
      viewport: Type.Optional(StringEnum(VIEWPORTS, { description: "Desktop 1280x720 or iPhone 13 mobile emulation" })),
      delayMs: Type.Optional(Type.Integer({ minimum: 0, maximum: 10_000, description: "Hold after visible actions; defaults to 2000" })),
    }),
    async execute(_id, { script, name, viewport, delayMs }, signal) {
      const result = await runtime.run(() => runtime.record(script, {
        name,
      ...
      return {
        content: [{ type: "text", text: await jsonOutput(result, "browser-record") }],
        details: result,
